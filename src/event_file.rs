...
pub trait FromByteStream:Default+Sized {
    fn read_from<R: Read>(reader: &mut R) -> Option<Self> {
        ...
        {
            let raw = unsafe { std::slice::from_raw_parts_mut((&mut result) as *mut Self as *mut u8, size) };
...
pub fn read_vec_from<T:Sized+Default+Clone, R:Read>(reader:&mut R, len:usize)->Option<Vec<T>>{
    let t_size=std::mem::size_of::<T>();
    let mut result=vec![T::default();len];
    let raw=unsafe{std::slice::from_raw_parts_mut(result.as_mut_ptr() as *mut u8, t_size*len)};
    match reader.read(raw){
...
impl FileHeader{
    pub fn new()->Self{
        ...
        FileHeader{basic_header:bh, additional_header:vec![0]}
    }

    pub fn with_additional_header(mut self, ah:Vec<u32>)->Self{
        let al=ah.len();
        assert!(!ah.is_empty());
        self.basic_header.length=((al-1)*std::mem::size_of::<u32>()+std::mem::size_of::<BasicFileHeader>()) as u32;
    ...
    pub fn read_from<R:Read>(reader:&mut R)->Option<FileHeader>{
        if let Some(bh)=BasicFileHeader::read_from(reader){
            let additional_length=(bh.length as usize-std::mem::size_of::<BasicFileHeader>())/std::mem::size_of::<u32>()+1;
            if let Some(additional_header)=read_vec_from(reader, additional_length){
                Some(FileHeader::new().with_basic_file_header(bh).with_additional_header(additional_header))
            }
            else{
                None
            }
...
impl EventHeader{
    pub fn from_trend_data(cont:&Data)->Self{
        EventHeader{
            header_length:0, runnr:0,
            eventnr: cont.event_count() as u32,
            t3eventnr:0,first_ls:0, event_sec:cont.sss() as u32,
            event_nsec:((4*cont.ts2()+cont.ts1pps() as u32-cont.ts1trigger() as u32) as f64*2.1) as u32,
            event_type:0, event_vers:0, ad1:0, ad2:0, ls_cnt:1
        }
    }
}
...
impl LocalStationHeader{
    pub fn from_trend_data(cont:&Data)->Self{
        LocalStationHeader{
            length:0,event_nr:cont.event_count() as u16,
            ls_id:(cont.ip()&0xffff) as u16,
            header_length:0,
            gps_seconds:cont.sss() as u32,
            gps_nanoseconds:((4*cont.ts2()+cont.ts1pps() as u32-cont.ts1trigger() as u32) as f64*2.1) as u32,
            trigger_flag:cont.trig_pattern() as u16,
            trigger_pos:0,sampling_freq:0, channel_mask:0, adc_resolution:12, trace_length:0, version:0
        }
    }
}
...
impl LocalStation{
    pub fn size(&self)->usize{
        std::mem::size_of_val(&self.header)+self.header_data.len()*std::mem::size_of::<u16>()+
            self.adc_buffer.len()*std::mem::size_of::<u16>()
    }

    pub fn new(lsh:LocalStationHeader, header_data:Vec<u16>, adc_buffer:Vec<u16>)->Self{
        let mut result=LocalStation{header:lsh, header_data:header_data, adc_buffer:adc_buffer};
        ...
        if let Some(h)=LocalStationHeader::read_from(reader){
            ...
            if let Some(header_data)=read_vec_from(reader, hl){
                if let Some(adc_buffer)=read_vec_from(reader, dl){
                    Some(LocalStation{
                        header:h,header_data:header_data, adc_buffer:adc_buffer
                    })
                }
                else{
                    None
...
impl Event{
    ...
    pub fn new(h:EventHeader)->Self{
        let mut result=Event{header:h, local_station_list:Vec::new()};
    ...
    pub fn from_trend_data(cont:&Data, adc_buffer:&Vec<u16>)->Self{
        let ls=LocalStation::new(LocalStationHeader::from_trend_data(cont), vec![], adc_buffer.clone());
    ...
    pub fn read_from<R:Read>(reader:&mut R)->Option<Event>{
        if let Some(h)=EventHeader::read_from(reader){
            ...
            Some(Event{header:h, local_station_list:lsl})
        }
        else{
            None
        }
...
impl EventFile{
    pub fn read_from<R:Read>(reader:&mut R)->Option<EventFile>{
        if let Some(h)=FileHeader::read_from(reader){
            ...
            Some(EventFile{header:h, event_list:el})
        }
        else{
            None
        }
