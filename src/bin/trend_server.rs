...
use std::io::Write;
use gp_daq::event_file::{FileHeader, Event};
...
fn main(){
    ...
    if args.len()>=3{
        ...
        let mut txt_file=OpenOptions::new().create(true).append(true).open(file_prefix+".yaml").unwrap();
        let file_prefix=args[2].clone();
        ...
        server.register_handler(Box::new(move |a,b|{
            match a{
                &TrendMsg::Data {ref content, ref payload}=>{
