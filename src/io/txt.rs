...
impl msg::TrendMsg{
    #[deprecated(note="the original cfg format will be unsupported soon, please switch to yaml asap")]
    pub fn write_to_txt<T:Write>(&self, write:&mut T, date:&DateTime<Utc>)->Result<(), std::io::Error>{
        match self{
            ...
            &msg::TrendMsg::Slc {ref content}=>{
                ...
                writeln!(write, "[Th1-,Th1+]: {} {}", decode_th(content.th1m()), decode_th(content.th1p()))?;
                writeln!(write, "[Th2-,Th2+]: {} {}", decode_th(content.th2m()), decode_th(content.th2p()))?;
                writeln!(write, "[Th3-,Th3+]: {} {}", decode_th(content.th3m()), decode_th(content.th3p()))?;
