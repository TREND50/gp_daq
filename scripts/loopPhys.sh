...
do
  ...
  if [ $BOARDID = "88" ]  || [ $BOARDID == "20" ]
  then
    ./phys.sh $BOARDID $thh $thh $thh $thh $thh $thh 001000 $NRUN  #Warning: ant_on = 0 in phys.sh
  else
    ./phys.sh $BOARDID $thl $thl $thl $thl $thl $thl 001000 $NRUN  #Warning: ant_on = 0 in phys.sh 
  fi
  sleep 1
