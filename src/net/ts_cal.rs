    ...
    pub fn update(&mut self, ip: IdT, sys_ts: f64, board_ts: f64) -> i64 {
        ...
        if self.cnt % 100 == 0 {
            let f = (self
                .shifts
                .iter()
                .map(|(_, v)| (Complex64::i() * (v - v.floor()) * 2.0 * f64::PI()).exp())
        ...
        let shift = match self.shifts.entry(ip) {
            Occupied(mut x) => {
                ...
                if ((old - self.frac_corr).round() - (y - self.frac_corr).round())
                    .abs()
                    .round() as i32
                    >= 1
                    && self.cnt > 100
                {
                    ...
                    eprintln!(
                        "ip={:?} sys_ts={} board_ts={} diff={} old={}  y={}, frac={}",
                        ip, sys_ts as u64, board_ts as u64, diff, old, y, self.frac_corr
                    );
                    //panic!();
                }
                if self.cnt % 100 == 0 {
                    let mut tsc_file = OpenOptions::new()
                        .create(true)
                        .append(true)
                        .open("tsdump.txt")
